...
#include "src/PitchShifter.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct PitchShifterMetaData {
		SN_NODE_ID("PitchShifter");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using PitchShifter = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _PitchShifter, PitchShifterMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
