/*
    Minimal third-party polyphonic sine probe with an unused audiofile slot.

    This exists to isolate whether simply having AudioFile complex data on a
    polyphonic compiled node is enough to trigger the garbled multi-note
    behaviour seen in spectral_player_loris.
*/
...
template <int NV> struct poly_sine_audio_probe : public data::base
{
	SNEX_NODE(poly_sine_audio_probe);

	struct MetadataClass
	{
		SN_NODE_ID("poly_sine_audio_probe");
	};
