...
#include "src/MODEFILTR.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct MODEFILTRMetaData {
		SN_NODE_ID("MODEFILTR");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using MODEFILTR = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _MODEFILTR, MODEFILTRMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
