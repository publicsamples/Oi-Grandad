template <int NV>
struct granular_player_stepquant_density_softcloud
{
    SNEX_NODE(granular_player_stepquant_density_softcloud);
    ...
    // -----------------------------------------------------
    // Deterministic density-scaled jitter (hybrid spread)
    inline double jitterOffset(int gi, int g, double scrubVal)
    {
        double jitterAmt = (double)(g - 1) / (double)(MAX_GRAINS - 1);
        jitterAmt *= 0.20; // up to 20% grainSize shift

        double s = (double)gi * 57.123 + scrubVal * 31.77;
        double r = Math.abs(Math.sin(s)) * 0.999; // stable random-ish 0..1

        double centered = r - 0.5; // -0.5..+0.5
    ...
    void processInternal(span<float, NUM_CHANNELS>& fd, VoiceData& v)
    {
        ...
        double spacing = grainSize / (double)g;

        for (int gi = 0; gi < g; gi++)
        {
            double offset = spacing * (double)gi;
            offset += jitterOffset(gi, g, v.scrubQ);
