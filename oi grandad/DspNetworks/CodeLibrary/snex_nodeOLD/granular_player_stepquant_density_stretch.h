template <int NV>
struct granular_player_stepquant_density_stretch
{
    SNEX_NODE(granular_player_stepquant_density_stretch);
	...
	double stretchMode = 0.0;   // 0 = normal, 1 = stretch
...
if (pitchMode < 0.5)
{
    ...
    grainPitchMul += spread * Math.sin(0.77); // grain 1 seed
}
else 
{
    double harmonicTarget = 1.0; // grain 1 = fundamental
    grainPitchMul = 1.0 + (harmonicTarget - 1.0) * pitchSpread;
}

double phaseInc1 = 0.0;

if (stretchMode < 0.5)
{
    phaseInc1 = v.delta * grainPitchMul;
}
else
{
        double stretchInc = grainSize / (stretchMs * sr * 0.001);
    phaseInc1 = stretchInc * grainPitchMul;
}

v.schedPhase += phaseInc1 * (1.0 + driftAmount);

double retrigMod1 = retrigAmount * Math.sin(v.schedPhase * 0.0007);
double effectiveSize1 =
    grainSize * (1.0 + collapse * 0.4 + density * 2.0);

if (v.schedPhase >= effectiveSize1 + retrigMod1)
{
...
pos1 += microSamples * Math.sin(0.71);

if (pos1 < 0.0) pos1 = 0.0;
if (pos1 >= audioData.numSamples - 1.0) pos1 = audioData.numSamples - 2.0;
...
double leftGain1  = 0.5 * (1.0 - pan1);
double rightGain1 = 0.5 * (1.0 + pan1);

Lsum += monoL1 * w1 * leftGain1 * weight1;
Rsum += monoR1 * w1 * rightGain1 * weight1;

// ----------------------------
// SECOND GRAIN (only if density > 1)
// ----------------------------
 ...
 if (g > 1)
{
    ...
    {
        v.schedActive2 = true;
        v.schedPhase2 = grainSize * 0.5 * density;
...
if (stretchMode < 0.5)
{
    phaseInc2 = v.delta * grainPitchMul2;
}
else
{
     double stretchInc2 = grainSize / (stretchMs * sr * 0.001);
    phaseInc2 = stretchInc2 * grainPitchMul2;
}

v.schedPhase2 += phaseInc2 * (1.0 + driftAmount);

    double retrigMod2 = retrigAmount * Math.sin(v.schedPhase2 * 0.0009);
    double effectiveSize2 = grainSize * (1.0 + collapse * 0.35 + density * 2.0);

    if (v.schedPhase2 >= effectiveSize2 + retrigMod2)
    {
    ...
    pos2 += microSamples * Math.sin(1.93);

    if (pos2 < 0.0) pos2 = 0.0;
    if (pos2 >= audioData.numSamples - 1.0) pos2 = audioData.numSamples - 2.0;
    ...
    double leftGain2  = 0.5 * (1.0 - pan2);
    double rightGain2 = 0.5 * (1.0 + pan2);

    Lsum += monoL2 * w2 * leftGain2 * weight2;
	Rsum += monoR2 * w2 * rightGain2 * weight2;
}

// ----------------------------
// THIRD GRAIN (if density > 2)
// ----------------------------
...
if (g > 2)
{
    ...
    {
        v.schedActive3 = true;
        v.schedPhase3 = grainSize * 0.25 * density;
...
double phaseInc3 = 0.0;

if (stretchMode < 0.5)
{
    phaseInc3 = v.delta * grainPitchMul3;
}
else
{
     double stretchInc3 = grainSize / (stretchMs * sr * 0.001);
    phaseInc3 = stretchInc3 * grainPitchMul3;
}

v.schedPhase3 += phaseInc3 * (1.0 + driftAmount);

    double retrigMod3 = retrigAmount * Math.sin(v.schedPhase3 * 0.0011);
    double effectiveSize3 = grainSize * (1.0 + collapse * 0.30 + density * 2.0);

    if (v.schedPhase3 >= effectiveSize3 + retrigMod3)
    {
    ...
    pos3 += microSamples * Math.sin(2.87);

    if (pos3 < 0.0) pos3 = 0.0;
    if (pos3 >= audioData.numSamples - 1.0) pos3 = audioData.numSamples - 2.0;
    ...
    double leftGain3  = 0.5 * (1.0 - pan3);
    double rightGain3 = 0.5 * (1.0 + pan3);

    Lsum += monoL3 * w3 * leftGain3 * weight3;
	Rsum += monoR3 * w3 * rightGain3 * weight3;
}
// ----------------------------
// FOURTH GRAIN (if density > 3)
// ----------------------------
...
{
    double spread = pitchSpread * spreadNorm;
    grainPitchMul4 += spread * Math.sin(3.57);
}
else
{
...
if (stretchMode < 0.5)
{
    phaseInc4 = v.delta * grainPitchMul4;
}
else
{
     double stretchInc4 = grainSize / (stretchMs * sr * 0.001);
    phaseInc4 = stretchInc4 * grainPitchMul4;
}

v.schedPhase4 += phaseInc4 * (1.0 + driftAmount);

    double retrigMod4 = retrigAmount * Math.sin(v.schedPhase4 * 0.0013);
    double effectiveSize4 = grainSize * (1.0 + collapse * 0.25 + density * 2.0);

    if (v.schedPhase4 >= effectiveSize4 + retrigMod4)
    {
    ...
    pos4 += microSamples * Math.sin(3.41);

    if (pos4 < 0.0) pos4 = 0.0;
    if (pos4 >= audioData.numSamples - 1.0) pos4 = audioData.numSamples - 2.0;
...
{
    double spread = pitchSpread * spreadNorm;
    grainPitchMul5 += spread * Math.sin(4.19);
}
else
{
...
double phaseInc5 = 0.0;

if (stretchMode < 0.5)
{
    phaseInc5 = v.delta * grainPitchMul5;
}
else
{
     double stretchInc5 = grainSize / (stretchMs * sr * 0.001);
    phaseInc5 = stretchInc5 * grainPitchMul5;
}

v.schedPhase5 += phaseInc5 * (1.0 + driftAmount);

    double retrigMod5 = retrigAmount * Math.sin(v.schedPhase5 * 0.0015);
    double effectiveSize5 = grainSize * (1.0 + collapse * 0.20 + density * 2.0);

    if (v.schedPhase5 >= effectiveSize5 + retrigMod5)
    {
    ...
    pos5 += microSamples * Math.sin(4.12);

    if (pos5 < 0.0) pos5 = 0.0;
    if (pos5 >= audioData.numSamples - 1.0) pos5 = audioData.numSamples - 2.0;
...
{
    double spread = pitchSpread * spreadNorm;
    grainPitchMul6 += spread * Math.sin(5.83);
}
else
{
...
double phaseInc6 = 0.0;

if (stretchMode < 0.5)
{
    phaseInc6 = v.delta * grainPitchMul6;
}
else
{
   double stretchInc6 = grainSize / (stretchMs * sr * 0.001);
    phaseInc6 = stretchInc6 * grainPitchMul6;
}

v.schedPhase6 += phaseInc6 * (1.0 + driftAmount);

    double retrigMod6 = retrigAmount * Math.sin(v.schedPhase6 * 0.0017);
    double effectiveSize6 = grainSize * (1.0 - collapse * 0.15 + density * 2.0);

    if (v.schedPhase6 >= effectiveSize6 + retrigMod6)
    {
    ...
    pos6 += microSamples * Math.sin(5.03);

    if (pos6 < 0.0) pos6 = 0.0;
    if (pos6 >= audioData.numSamples - 1.0) pos6 = audioData.numSamples - 2.0;
...
{
    double spread = pitchSpread * spreadNorm;
    grainPitchMul7 += spread * Math.sin(6.61);
}
else
{
...
double phaseInc7 = 0.0;

if (stretchMode < 0.5)
{
    phaseInc7 = v.delta * grainPitchMul7;
}
else
{
     double stretchInc7 = grainSize / (stretchMs * sr * 0.001);
    phaseInc7 = stretchInc7 * grainPitchMul7;
}

v.schedPhase7 += phaseInc7 * (1.0 + driftAmount);

    double retrigMod7 = retrigAmount * Math.sin(v.schedPhase7 * 0.0019);
    double effectiveSize7 = grainSize * (1.0 + collapse * 0.10 + density * 2.0);

    if (v.schedPhase7 >= effectiveSize7 + retrigMod7)
    {
    ...
    pos7 += microSamples * Math.sin(6.44);

    if (pos7 < 0.0) pos7 = 0.0;
    if (pos7 >= audioData.numSamples - 1.0) pos7 = audioData.numSamples - 2.0;
...
{
    double spread = pitchSpread * spreadNorm;
    grainPitchMul8 += spread * Math.sin(7.49);
}
else
{
...
double phaseInc8 = 0.0;

if (stretchMode < 0.5)
{
    phaseInc8 = v.delta * grainPitchMul8;
}
else
{
      double stretchInc8 = grainSize / (stretchMs * sr * 0.001);
    phaseInc8 = stretchInc8 * grainPitchMul8;
}

v.schedPhase8 += phaseInc8 * (1.0 + driftAmount);

    double retrigMod8 = retrigAmount * Math.sin(v.schedPhase8 * 0.0021);
    double effectiveSize8 = grainSize * (1.0 - collapse * 0.05 + density * 2.0);

    if (v.schedPhase8 >= effectiveSize8 + retrigMod8)
    {
    ...
    pos8 += microSamples * Math.sin(7.91);

    if (pos8 < 0.0) pos8 = 0.0;
    if (pos8 >= audioData.numSamples - 1.0) pos8 = audioData.numSamples - 2.0;
    ...
    double normPan = ((8.0 - center) * invDenom);
